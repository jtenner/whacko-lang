...
import { CompileTimeBool, CompileTimeValue, CompileTimeFloat, CompileTimeInteger, CompileTimeInvalid, CompileTimeString, ExecutionContext, ExecutionContextValue, RuntimeValue, RuntimeInvalid, RuntimeBool, RuntimeString, RuntimeFloat, RuntimeInteger } from "../execution-context";
import { BinaryExpression, BinaryLiteral, ExpressionStatement, FloatLiteral, FunctionDeclaration, HexLiteral, IntegerLiteral, isFunctionDeclaration, StringLiteral, isID, isMemberAccessExpression, MemberAccessExpression } from "../generated/ast";
import { ClassType, ConcreteType, DynamicTypeScopeElement, FloatType, IntegerType, InvalidType, ScopeTypeElement, StaticTypeScopeElement, Type } from "../types";
...
export class CompilationPass extends WhackoPass {
  ...
  compileElement(func: ScopeTypeElement, typeParameters: ConcreteType[] | null) {
    assert(isFunctionDeclaration(func.node), `Expected function in FunctionCompilationPass.`);
    ...
    if (func instanceof StaticTypeScopeElement) {
      ...
      if (typeParameters?.length !== func.typeParameters.length) {
        this.error("Semantic", func.node, `Expected ${func.typeParameters.length} parameters.`);
        return;
      }
    }
  ...
  override visitIntegerLiteral(expression: IntegerLiteral): void {
    ...
    if (value <= 9_223_372_036_854_775_807n && value >= -9_223_372_036_854_775_808n) {
      this.currentCtx.stack.push(new CompileTimeInteger(value, new IntegerType(Type.i64, value, expression)));
    } else if (value >= 0n && value <= 18_446_744_073_709_551_615n) {
      this.currentCtx.stack.push(new CompileTimeInteger(value, new IntegerType(Type.u64, value, expression)));
    } else {
      this.error("Semantic", expression, "Invalid number, must be a valid i64 or u64.");
    }
  }

  override visitFloatLiteral(expression: FloatLiteral): void {
    const value = parseFloat(expression.value);
    this.currentCtx.stack.push(new CompileTimeFloat(value, new FloatType(Type.f64, value, expression)));
  }
  ...
  override visitBinaryExpression(node: BinaryExpression): void {
    ...
    if (left.ty.isAssignableTo(right.ty)) {
      if (left instanceof CompileTimeValue && right instanceof CompileTimeValue) {
        // we can concatenate the two items
        this.currentCtx.stack.push(new CompileTimeBool(left.value === right.value, node));
      } else {
    ...
    if (isID(node.lhs)) {
      ...
      if (local instanceof RuntimeValue) {
        ...
        if (rhs.valid) {
          ...
          ctx.vars.set(node.lhs.name, new RuntimeInvalid(new InvalidType(node)));
        }
        ...
        this.error("Semantic", node.lhs, "Invalid left hand side, variable is constant.");
        ctx.stack.push(new CompileTimeInvalid(node));
      }
      ...
      assert(rootElement, "Could not pop context stack element because the stack was exhausted.");
      this.visit(node.rhs);
      const valueElement = this.currentCtx.stack.pop()!;
      assert(valueElement, "Could not pop context stack element because the stack was exhausted.");
      ...
      if (rootElement.ty instanceof ClassType) {
        ...
        this.error("Type", mas.member, `Invalid type, ${rootElement.ty.name} is not a class.`);
      }
    } else {
      // this.visit(node.lhs);
      this.visit(node.rhs);
      this.error("Semantic", node.lhs, "Invalid left hand side, invalid expression.");
      ctx.stack.pop();
      ctx.stack.push(new CompileTimeInvalid(node));
    }
  }
